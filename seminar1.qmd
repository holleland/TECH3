...
#| eval: false
set.seed(1)
x <- round(rnorm(11, mean = 5),1)
print(sort(x))
```

Consider these 11 sorted numbers: 
$$4.2, 4.2, 4.4, 4.7, 5.2, 5.3, 5.5, 5.6, 5.7, 6.5, 6.6.$$

a) For the given set of data, find the i) Median, ii) 1st and 3rd quartile, iii)  Interquartile range, iv) Min and Max.


b)  Use the information from (a) to draw a box plot of the data points.
...
$$f(x) = \lambda e^{-2x}, \quad x\ge 0.$$

a) Find $\lambda$ for $f$ to fulfill the requirements of being a density. 

b) Find an expression for the cumulative distribution function $F(x)=P(X\le x)$. 

c) What is the probability that X is larger than 2?

d) Show that the distribution of X fulfills the memoryless property, i.e. that 
$$P(X>s+t|X>s)=P(X>t).$$ 

e) X is exponentially distributed, which in general, has the memoryless property. This distribution is often used for waiting times. Can you give an intuitive interpretation of the memoryless property if X for example is the waiting time for the next arriving bus and you condition on already having waited $s$ minutes?

<!-- ### Exercise 6 (Trial exam Spring 2025, Problem 3a-b) -->

<!-- ::::: columns -->
<!-- ::: {.column width="20%"} -->
<!-- ![](images/milkjug.png){width="90%"} -->
<!-- ::: -->
