...
```{r, echo = FALSE, output = "asis", message = FALSE, warning = FALSE}
library(knitr)
...
xtable::xtable(prob.table,digits = 0) %>% print(include.rownames=TRUE, type="html")
```
...
prob.table <- tibble(Outcomes, 
  ...
  mutate(.row = "Probability") %>%
  column_to_rownames(".row")
xtable::xtable(prob.table,digits = 0) %>% 
  print(include.rownames=TRUE, type="html", hline.after = 1)
```
...
tibble("Institute" =  c("A", "B", "C", "D", "E", "F"),
  ...
  t() %>% 
  xtable() %>% print(include.colnames = F, hline.after = 1, type="html")
```
...
![Probability Tree](images/FigurTree.png)
