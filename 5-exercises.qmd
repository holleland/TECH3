...
a)  We essentially treat the best fit line in a linear regression like we would any linear function, where we have a set formula for the slope, $hat{\beta_1}=\frac{\hat{Cov(X,Y)}}{S^2_X}$, and $\hat{\beta_0}=\bar{Y}-\beta_1\bar{X}$, and gives the intercept.
...
\hat{Cov(X,Y)}=\frac{1}{2}(1*\frac{5}{6}+0+1*\frac{7}{6})=\frac{1}{2}\frac{12}{6}=1
\end{align*}
...
$$ What we would like now is the conditional expected value of $Y$ when $X=4$. Recall recall that $E[\varepsilon]=0$, and $\varepislon$ is also independent of $X$, meaning $E[\varepsilon|X]=E[\varepsilon]$
...
```{r}
df <- data.frame(score = c(80, 85, 70, 90),
                 hours = c(4,5,3,6),
                 female = factor(c(0, 1, 0, 1)))
model <- lm(score ~ hours + female, data = df)
summary(model)
```
...
#| eval: false
#| echo: false
## Use this to generate the necessary data
set.seed(42)
price <- runif(100, 1, 20)
log_odds <- -0.5 * price + 3
prob_buy <- 1 / (1 + exp(-log_odds))
buy <- rbinom(100, 1, prob_buy)
df <- data.frame(price, buy)
```
...
#| eval: false
library(ggplot2)
ggplot(df, aes(x = price, y = buy)) +
  geom_point(alpha = 0.5) +
  stat_smooth(method = "glm", method.args = list(family = "binomial"),
              se = FALSE, color = "red", linewidth = 1.2) +
  labs(title = "Logistic Regression: Purchase vs Price",
       x = "Price", y = "Probability of Purchase") +
  theme_minimal()
```
...
-   At price = 10, the predicted probability of purchase is around 0.05 (depending on fit). The red curve shows how this probability changes with price.
...
# Simulate data
set.seed(123)
budget <- seq(0, 20, by = 0.5)
demand <- 10 + 4 * budget - 0.2 * budget^2 + rnorm(length(budget), 0, 1)
data <- data.frame(budget, demand)

# Fit linear model
model0 <- lm(demand ~ budget, data=data)
summary(model0)

# Fit quadratic model
model1 <- lm(demand ~ budget + I(budget^2), data = data)
summary(model1)

# Visualize both
ggplot(data, aes(x = budget, y = demand)) +
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x, se = FALSE) +
  labs(x = "Budget", y = "Demand")
...
# Model B: Polynomial (y=x+x^2+...+x^10)
model_b <- lm(y ~ poly(x, 10), data = data)
summary(model_b)
```
...
c)  Cross-validation assesses performance on unseen data, discouraging overfitting.I.e., you validate a model by using ity on more than just the data it was built on.
...
Given: $\text{score} = 50 + 2 \cdot \text{study_hours}$ You calculate:
