...
a)  The statement of the instructor is that the mean is **less** than 70 bpm, so that becomes our alternative hypothesis, and the oppsite case becomes the null hypothesis. We use \$H_0: \mu=70 \$ as we are only interested in the edge case when testing.
...
c)  $t = \frac{68 - 70}{2.45/\sqrt{10}} \approx -2.45$
...
c)  Use a z-transformation to b) to standard normal, and compute using a calculator. Let $Z\sim \matchal{N}(0,1)$, we call that a standard normal distribution. We also have that
...
\$\$ \bar{X}=\frac{1}{n}\sum\^n\_{i=1}X_i, \quad S^2=^\frac{1}{n-1}\sumn\_{i=1}(X_i-\bar{X})\^2
...
\$\$ \lim*{n*\rightarrow\infty}S^2=^\sigma2 \quad  \Rightarrow \quad  \frac{\bar{X}-\mu}{S/\sqrt{n}}\sim t{n-1} \rightarrow \frac{\bar{X}-\mu}{\sigma/\sqrt{n}}\sim\mathcal{N}(0,1)
...
a)  CLT tells us that $\bar{X} \sim \mathcal{N}\left(10, \frac{5}{\sqrt{50}}\right)$
...
# Sample sizes to try
sample_sizes = [5, 20, 80, 320, 1280]
bins = [30, 30, 20, 50, 100]
...
c)  For a two-sided *t*-test at $\alpha=0.05$, this checks if test statistic is significant at 5% level for df = 10.
